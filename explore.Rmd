...
adult_eig <- read_csv(here("simulations/cluster_results/exposure_duration/adults/aligned/summarized_output_EIG.csv"))
trial_info <- read_csv(here("simulations/cluster_results/exposure_duration/adults/sim_info/trial_info/trial_info_exposure_duration.csv"))
...
eig_summarized <- trial_info %>% 
  ...
  right_join(adult_eig, by = c("trial_id", "stim_id")) %>% 
  ...
  select(param_id, trial_type, fam_duration, trial_number, sample_n_mean, sample_n_std) %>% 
...
eig_summarized = trial_info %>% 
  ...
  mutate(trial_type = violation_type, 
         trial_type = if_else(trial_type == "identity", "deviant", "background")) %>% 
  group_by(param_id, trial_type, fam_duration) %>% 
  summarise(
    count = n(),
    #ssd = sum((20-1) * (sample_n_std ** 2)),
    #sd_sample = (ssd / 20 * count - count) ** .5, 
...
run_k_fold <- function(p_id, raw_d, sd){
  ...
  for (k in 1:n_folds){
    ...
    train_data = summarize_behavioral_data(raw_d %>% filter(prolific_id %in% train_subject)) %>% ungroup() %>% left_join(sd %>% filter(param_id == p_id) %>% ungroup(), by = c("fam_duration", "trial_type")) %>% 
      filter(trial_number != 11)
    test_data = summarize_behavioral_data(raw_d %>% filter(!prolific_id %in% train_subject)) %>% ungroup() %>% left_join(sd %>% filter(param_id == p_id) %>% ungroup(), by = c("fam_duration", "trial_type")) %>% 
      filter(trial_number != 11)
...
full_k_fold_res <- lapply(
  ...
  function(id){
    run_k_fold(p_id = id, 
               raw_d = bd %>% filter(trial_number != 11, exposure_duration != 10), 
               sd = eig_summarized %>% filter(fam_duration != 10))
...
full_k_fold_res %>% 
  ...
  geom_histogram() + 
   labs(title = "by fam duration, mean r square")

eig_summarized %>% 
  ggplot(aes(x = trial_number, y = mean_sample, color = trial_type)) + 
  stat_summary(fun.data = "mean_cl_boot", position = position_dodge(width = .2))+ 
  theme_few() + 
  labs(title = "by trial number, model fits")
...
bd %>% 
  ...
  stat_summary(fun.data = "mean_cl_boot", position = position_dodge(width = .2))+ 
  theme_few() + 
  labs(title = "by fam duration, behavioral")
...
eig_summarized %>% 
  ...
  stat_summary(fun.data = "mean_cl_boot", position = position_dodge(width = .2)) + 
  theme_few() + 
  labs(title = "by fam duration, best fits")
```
