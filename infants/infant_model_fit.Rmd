...
# unscaled all stims
model_data = read_csv(here('simulations/sim_infant_exposure_duration/01_linked_sim/linked_aligned_eig.csv')) 

# remove the trial 0 of novel
model_data = model_data %>% filter(!(trial_number == 1 & trial_type == 'deviant'))
...
model_data %>% filter(param_id == 107) %>% ggplot(aes(x = trial_number, y = mean_sample, color = test_type)) +
 stat_summary(fun.data = "mean_cl_boot") + geom_line() + theme_minimal(20) + ggtitle('EIG, param id = 107')
...
model_data <- model_data %>%
  ...
  filter(all(2:10 %in% trial_number)) %>%
  ungroup() %>%
...
# Crossvalidation (run above blocks first)
```{r}
